...
OUT = ROOT / "renders" / "glass_reflection_visibility_v1B"
REP = ROOT / "reports" / "glass_reflection_visibility_v1B"
...
def make_plane_mesh(name, width, height):
...
def safe_remove_matching_objects():
    removed = []
    # Collect names first. Do not access object properties after bpy.data.objects.remove().
    names = []
    for o in bpy.data.objects:
        n = o.name
        if n.startswith("FX_ReflectCard_") or n.startswith("FX_GlassStreak_") or n.startswith("FX_WindowGlow_") or "White_Headlight" in n:
            names.append(n)
    for name in names:
...
def create_or_replace_plane(name, width, height):
    old = bpy.data.objects.get(name)
    if old:
        bpy.data.objects.remove(old, do_unlink=True)
    mesh = make_plane_mesh(name, width, height)
...
def setup_reflection_cards_and_streaks(gb):
    ...
    for name, x, y, z, w, h, mat in card_specs:
        obj = create_or_replace_plane(name, w, h)
...
def write_reports(removed, gbdata, under, traffic, fx, glass, fx_scan, lights):
    ...
    payload = {"old_reflection_fx_removed": removed, "glass_objects": gbdata["objects"], "glass_bounds": gbdata["bounds"], "underglow_lock": under,
               ...
               "note": "v1B fixes the Object-removed iteration bug. This pass uses camera-invisible reflection cards plus subtle on-glass streak overlays for reliable visible red/yellow/green reflections."}
    (REP/"glass_reflection_visibility_v1B.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    lines = ["# Glass Reflection Visibility v1B", "",
             "## What changed",
    ...
    (REP/"Glass_Reflection_Visibility_v1B.md").write_text("\n".join(lines), encoding="utf-8")
...
def manifest():
    ...
    (AUD/"project_file_layout.json").write_text(json.dumps({"generated_by":"glass_reflection_visibility_v1B","reports":str(REP),"renders":str(OUT),"current_review":str(CUR)}, indent=2), encoding="utf-8")
...
def render_review(gb):
    ...
    hc = center(hb)
    cams = [
        temp_cam("TMP_GlassReflection_Close_v1B", Vector((target.x + gb["dim_x"]*0.45, gb["min_y"] - 5.0, target.z + 0.65)), target, 58),
        temp_cam("TMP_GlassReflection_Oblique_v1B", Vector((target.x - gb["dim_x"]*0.45, gb["min_y"] - 6.2, target.z + 0.80)), target, 48),
        temp_cam("TMP_GlassReflection_FXSetup_v1B", Vector((target.x, gb["min_y"] - 8.0, target.z + 1.1)), target, 42),
        temp_cam("TMP_GlassReflection_CharacterReady_v1B", Vector((hc.x+2.2,hc.y-6.0,hc.z+1.5)), Vector((hc.x,hc.y,hc.z+0.45)), 55),
    ]
    old = scene.camera
...
def main():
    reset()
    log("[pass] glass reflection visibility v1B")
    removed = safe_remove_matching_objects()
    gbdata = glass_bounds()
    gb = gbdata["bounds"]
