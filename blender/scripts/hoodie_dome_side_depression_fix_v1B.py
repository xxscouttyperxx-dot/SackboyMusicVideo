...
REP = ROOT / "reports" / "hoodie_dome_side_depression_fix_v1B"
...
NEW_HOODIE_KEY = "HOODIEFIT_DomeSideDepressionFix_v1B"
...
def log(msg):
    ...
    with (REP / "HoodieDomeSideDepressionFix_v1B_report.txt").open("a", encoding="utf-8") as f:
        f.write(msg + "\n")
...
def reset_outputs():
    ...
    for p in OUT.glob("*"):
        if p.is_file():
            p.unlink()
    (REP / "HoodieDomeSideDepressionFix_v1B_report.txt").write_text("", encoding="utf-8")
...
def smooth_region(key, adjacency, weights, factor=0.36):
...
def apply_dome_side_depression_fix(hoodie):
    ...
    counts = {
        "side_depression_rounded": 0,
        "back_droop_rounded": 0,
        "rear_protrusion_feathered": 0,
        "shoulder_collar_raised": 0,
        "front_rim_preserved": 0,
        "top_crown_feathered": 0,
    }
    ...
    for i, p in enumerate(key.data):
        ...
        extreme_side = smoothstep(0.54, 0.92, nx)
        center = radial(nx, ny, 0.82, 0.92)
        wide = radial(nx, ny, 1.12, 1.08)
        rear = smoothstep(0.14, 0.78, ny_signed)
        front = smoothstep(0.14, 0.78, -ny_signed)

        top_crown = band(zn, 0.70, 0.80, 1.00, 1.00)
        upper_side = band(zn, 0.52, 0.62, 0.84, 0.94)
        mid_side = band(zn, 0.42, 0.52, 0.72, 0.84)
        lower_side = band(zn, 0.32, 0.42, 0.62, 0.78)
        shoulder_collar = band(zn, 0.42, 0.52, 0.68, 0.82)
        back_mid = band(zn, 0.38, 0.50, 0.76, 0.92)
        rear_top = band(zn, 0.58, 0.68, 0.90, 1.00)
        front_rim = (upper_side + mid_side + lower_side) * front * (0.55 + 0.45*side)

        new = co.copy()

        side_depress_w = (0.58*upper_side + 0.78*mid_side + 0.60*lower_side) * side * (1.0 - 0.55*front) * (0.65 + 0.35*rear)
        if side_depress_w > 0:
            new.x = cx + (new.x - cx) * (1.0 + 0.040 * side_depress_w)
            new.y = cy + (new.y - cy) * (1.0 + 0.018 * side_depress_w)
            new.z -= dz * 0.010 * lower_side * side * (1.0 - 0.45*front)
            smooth_weights[i] = max(smooth_weights[i], 0.70 * side_depress_w)

        back_droop_w = back_mid * rear * (0.45 + 0.55*wide)
        if back_droop_w > 0:
            new.z += dz * 0.032 * back_droop_w
            new.y = cy + (new.y - cy) * (1.0 + 0.010 * back_droop_w)
            smooth_weights[i] = max(smooth_weights[i], 0.62 * back_droop_w)

        rear_bump_w = rear_top * rear * (0.45 + 0.55*wide)
        if rear_bump_w > 0:
            new.y = cy + (new.y - cy) * (1.0 - 0.018 * rear_bump_w)
            new.z += dz * 0.014 * rear_bump_w
            smooth_weights[i] = max(smooth_weights[i], 0.58 * rear_bump_w)

        collar_w = shoulder_collar * extreme_side * (1.0 - 0.35*front)
        if collar_w > 0:
            new.z += dz * 0.026 * collar_w
            new.x = cx + (new.x - cx) * (1.0 + 0.012 * collar_w)
            smooth_weights[i] = max(smooth_weights[i], 0.48 * collar_w)

        crown_w = top_crown * (0.45 + 0.55*center)
        if crown_w > 0:
            new.z += dz * 0.008 * crown_w
            smooth_weights[i] = max(smooth_weights[i], 0.42 * crown_w)

        rim_w = min(1.0, front_rim)
        if rim_w > 0.08:
            smooth_weights[i] = max(smooth_weights[i], 0.18 * rim_w)

        delta = (new - co).length
        if delta > 1e-7:
            if side_depress_w > 0.08: counts["side_depression_rounded"] += 1
            if back_droop_w > 0.08: counts["back_droop_rounded"] += 1
            if rear_bump_w > 0.08: counts["rear_protrusion_feathered"] += 1
            if collar_w > 0.08: counts["shoulder_collar_raised"] += 1
            if rim_w > 0.08: counts["front_rim_preserved"] += 1
            if crown_w > 0.08: counts["top_crown_feathered"] += 1
            p.co = new
            touched += 1
            max_delta = max(max_delta, delta)

    adjacency = build_adjacency(hoodie.data)
    smoothed_count, max_smooth = smooth_region(key, adjacency, smooth_weights, factor=0.36)
    ...
    hoodie["hoodie_fit_pass"] = "HoodieDomeSideDepressionFix_v1B"
...
def cleanup_and_create_cameras(hoodie):
    ...
    set_cam("CAM_F2_Front", (fmid.x, fmid.y - 6.0, fmid.z + 0.55), fmid, 55, False)
    set_cam("CAM_F2_Profile", (fmid.x + 5.5, fmid.y - 0.6, fmid.z + 0.55), fmid, 60, False)
    set_cam("CAM_F2_ThreeQuarter", (fmid.x + 3.8, fmid.y - 5.8, fmid.z + 0.75), fmid, 52, False)
    set_cam("CAM_StorefrontReflection", (hc.x + 7.8, hc.y - 10.0, hc.z + 1.45), (hc.x, hc.y + 4.2, hc.z + 0.65), 48, False)
    set_cam("CAM_SceneWide", (hc.x + 8.5, hc.y - 10.5, hc.z + 4.2), (hc.x, hc.y + 1.0, hc.z + 0.3), 35, False)
    set_cam("CAM_Hood_LeftSide", (hood_mid.x - 3.35, hood_mid.y, hood_mid.z + 0.58), hood_mid + Vector((0,0,0.14)), 58, False)
    set_cam("CAM_Hood_RightSide", (hood_mid.x + 3.35, hood_mid.y, hood_mid.z + 0.58), hood_mid + Vector((0,0,0.14)), 58, False)
    set_cam("CAM_Hood_Top", (hood_top.x + 0.05, hood_top.y - 0.20, hood_top.z + 3.20), hood_top, 72, False)
    set_cam("CAM_Hood_Front", (hood_mid.x + 0.20, hood_mid.y - 3.70, hood_mid.z + 0.95), hood_mid + Vector((0,0,0.08)), 56, False)
    set_cam("CAM_ANIM_Wide", (hc.x + 9.0, hc.y - 11.5, hc.z + 4.0), (hc.x, hc.y + 0.8, hc.z + 0.3), 32, True)
    set_cam("CAM_ANIM_Medium", (hc.x + 5.0, hc.y - 7.0, hc.z + 2.3), (hc.x, hc.y + 0.4, hc.z + 0.3), 42, True)
    set_cam("CAM_ANIM_Close", (hc.x + 2.2, hc.y - 4.0, hc.z + 1.5), (hc.x, hc.y + 0.1, hc.z + 0.4), 60, True)
    ...
    try:
        ...
        for cam_name, filename, color_type, isolate, wire in specs:
            ...
            if wire:
                ...
                cam.location = cam.location + direction * 1.10
                look_at(cam, center)
...
def write_reports(hoodie_fit, camera_report, disabled_bodyfit, under, renders):
    payload = {
        "pass": "hoodie_dome_side_depression_fix_v1B",
        ...
        "notes": [
            "Text/json reports are now stored only under reports/hoodie_dome_side_depression_fix_v1B.",
    ...
    (REP / "HoodieDomeSideDepressionFix_v1B_status.json").write_text(json.dumps(status, indent=2), encoding="utf-8")
    (REP / "hoodie_dome_side_depression_fix_v1B.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    md = [
        "# Hoodie Dome Side Depression Fix v1B",
        "",
        "## Deformation Summary",
        "This pass focuses on correcting hood side depressions and back silhouette flow.",
        "The hood should read as a smooth half-dome/bowl around Sackboy's head while preserving the front face opening.",
        "The front rim remains the only intentional break in the bowl shape.",
        ...
        "- Text/json outputs are in `reports/hoodie_dome_side_depression_fix_v1B`.",
    ]
    (REP / "Hoodie_Dome_Side_Depression_Fix_v1B.md").write_text("\n".join(md), encoding="utf-8")
...
def manifest():
    ...
    (AUD / "project_file_layout.json").write_text(json.dumps({"generated_by": "hoodie_dome_side_depression_fix_v1B", "reports": str(REP), "current_review": str(OUT)}, indent=2), encoding="utf-8")

def main():
    reset_outputs()
    log("[pass] hoodie dome side depression fix v1B")
    hoodie = find_hoodie()
    under = restore_underglow()
    disabled_bodyfit = keep_character_baseline()
    hoodie_fit = apply_dome_side_depression_fix(hoodie)
...
if __name__ == "__main__":
    try:
        ...
        REP.mkdir(parents=True, exist_ok=True)
        with (REP / "HoodieDomeSideDepressionFix_v1B_FATAL_ERROR.txt").open("w", encoding="utf-8") as f:
