...
OUT_DIR = PROJECT_ROOT / "renders" / "decimate_preview_paint_snap_v1"
REPORT_DIR = PROJECT_ROOT / "reports" / "decimate_preview_paint_snap_v1"
...
def parking_paint_objects():
    ...
    for obj in bpy.data.objects:
        if obj.type != "MESH":
            continue
        text = (obj.name + " " + " ".join(c.name for c in obj.users_collection)).lower()
        if any(k in text for k in ["hparking", "parking", "stripe", "strip", "paint", "line", "divider", "spine"]):
            if any(bad in text for bad in ["lamp", "sign", "sidewalk", "curb", "storefront", "camera"]):
                continue
...
def write_reports(decimate, paint):
    ...
    (REPORT_DIR / "decimate_preview_paint_snap_v1.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")

    lines = ["# Decimate Preview / Paint Snap v1", ""]
...
def write_scene_manifest():
    ...
    (AUDIT_DIR / "scene_layout_summary.md").write_text(
        ...
        "Updated by Decimate Preview / Paint Snap v1.\n\n"
    ...
    (AUDIT_DIR / "project_file_layout.json").write_text(json.dumps({
        "generated_by": "decimate_preview_paint_snap_v1",
