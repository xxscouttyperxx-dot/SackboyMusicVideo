...
Write-Host "=== CHARACTER SPATIAL / HIERARCHY AUDIT V1 ==="
Write-Host "Mode: strictly read-only. No scene edits, no shape-key edits, no origin changes, no parenting changes, no .blend save."
...
if(!(Test-Path ".\blender\scripts\character_spatial_hierarchy_audit_v1.py")){throw "Missing audit script"}
...
& $BlenderExe --background ".\blender\sackboy_scene.blend" --python-exit-code 1 --python ".\blender\scripts\character_spatial_hierarchy_audit_v1.py"
if($LASTEXITCODE -ne 0){throw "Blender character spatial/hierarchy audit failed with exit code $LASTEXITCODE"}
...
Write-Host "Reports: reports\character_spatial_hierarchy_audit_v1"
Write-Host "No .blend save detected."
Write-Host "No scene changes detected."
